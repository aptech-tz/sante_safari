...
const TESTIMONIALS = [
  {
    name: "Amina K.",
    text: "Sante Safaris gave me the most peaceful and healing experience of my life. I returned home truly rejuvenated!"
  },
  {
    name: "James M.",
...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="bg-blue-200 py-4">
      <div className="max-w-4xl mx-auto px-6">
        <h2 className="text-3xl font-bold text-center text-sky-700 mb-10">Testimonials</h2>
            ...
            >
              {TESTIMONIALS.map((t, i) => (
                <div key={i} className="w-full flex-shrink-0 px-4">
                  <blockquote className="bg-white rounded-xl shadow-md p-8 flex flex-col items-center text-center min-h-[200px]">
                    <p className="text-lg italic text-gray-700 mb-6">"{t.text}"</p>
                    <span className="font-bold text-sky-600">— {t.name}</span>
                  </blockquote>
                </div>
              ))}
            </div>
            ...
            {TESTIMONIALS.map((_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors duration-200 ${
                  i === currentSlide ? 'bg-sky-600' : 'bg-blue-100 hover:bg-blue-200'
                }`}
